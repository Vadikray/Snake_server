...
export class State extends Schema {
    ...
    collectApple(player: Player, data: any){
        ...
        player.score++;
        player.d = Math.floor(player.score / 3);
    }
    ...
    gameOver(data){
        ...
        const cliendID = detailsPositions.id

        const gameOverID = this.gameOverIDs.find((value) => value === cliendID)
        if(gameOverID !== undefined) return;
        this.gameOverIDs.push(cliendID);
        this.delayClearGameOverIDS(cliendID);

        this.removePlayer(cliendID);
    ...
    async delayClearGameOverIDS(cliendID){
        await new Promise(resolve => setTimeout(resolve, 10000));

        const index = this.gameOverIDs.findIndex((value) => value === cliendID)
        if(index <= -1) return;
